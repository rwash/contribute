...
describe CompletedProjects do
	describe "integration tests" do
		...
		it "run works" do
			funded = FactoryGirl.build(:project, :funding_goal => 10, :end_date => (Date.today - 1))
			#Validation prevents projects with an end_date before or on today. So skip validation.
			funded.save(:validate => false)
			not_funded = FactoryGirl.build(:project2, :funding_goal => 100000, :end_date => (Date.today - 1))
...
#			to_funded2.should_receive(:execute_payment).once
		...
		it "run all works" do
			funded = FactoryGirl.build(:project, :funding_goal => 15, :end_date => (Date.today - 1))
			#Validation prevents projects with an end_date before or on today. So skip validation.
			funded.save(:validate => false)
			not_funded = FactoryGirl.build(:project2, :funding_goal => 100000, :end_date => (Date.today - 1))
			not_funded.save(:validate => false)
			funded_not_ignored = FactoryGirl.build(:project3, :funding_goal => 95, :end_date => (Date.today - 3))
